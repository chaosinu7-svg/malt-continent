...
> ⚠️ 重要变更（2026-07-03）：**珍稀度改由网页 UI 表达——宝石颜色分级 + 光晕**（炉石式）：★1 白水晶 / ★2 翡翠绿 / ★3 蓝宝石 / ★4 紫水晶 / ★5 琥珀金，宝石色同时驱动卡片光晕颜色（★3 静态微光 / ★4 脉动 / ★5 圣光呼吸），已在站上做好。所以瓶身图**统一画成干净明亮的产品图即可，不要再烤五档暗光/圣光/宝石进图里**——珍稀度网页已经表达了，图里再烤会不一致、打架。统一光照 = 更好出、更成套。

### 每瓶怎么画
1. 先读 `src/content/whiskies/<id>.md`，拿 `name`（中文名）、`name_en`（英文名，做酒标文字用）、`cask`（桶型）、`flavor_tags`（风味）
2. 画瓶身：瓶型、酒标、酒液颜色贴合真实产品特征（拉弗格白底黑字、麦卡伦深色雪莉桶瓶、美格红蜡封口、尊尼获加方瓶斜标）
3. 统一光照：柔和明亮的漫射光/单侧窗光，羊皮纸底，水彩钢笔线稿。**所有瓶子一套光，不分档。**
