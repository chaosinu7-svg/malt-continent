...
- 同档瓶子的光必须长得一样（同模板换瓶），否则梯度才清楚；建议每档先出 1 张定版，再批量
- **酒标必须有字**：提示词里用引号明确写出标签文字（英文/罗马字，即 md 的 `name_en`，如 `酒标印着 "LAPHROAIG" "10 YEARS"`），别写中文标签（会糊）；糊了就重生成那一张。中文名由网页渲染，不必画进图
- **不要画宝石/星星到图里**：珍稀度网页已用星级角标 + 琥珀光晕表达（曾试过宝石分级，已撤）；图里只负责上面这套「明暗度」
...
**产区横幅重画待办（2026-07-03）：** 现有 6 张横幅在页面上观感偏拉伸。站点 CSS 已锁死容器为 16:5、与 1600×500 原图完全一致（不再由前端引入变形），所以问题在素材本身——出图时画面内容未严格填满 3.2:1 画幅或被压扁。最后统一重画时：① 画布严格 1600×500，构图就按 3.2:1 全景来（别在方图上硬拉）；② 落盘后可让 Claude 起预览核验 dispRatio≈3.20 且观感自然。
